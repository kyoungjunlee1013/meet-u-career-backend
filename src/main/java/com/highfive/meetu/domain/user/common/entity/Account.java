...
import com.highfive.meetu.domain.user.common.type.AccountTypes.Status;
import com.highfive.meetu.domain.user.common.type.AccountTypes.Type;
import com.highfive.meetu.domain.user.common.type.AccountTypes.OAuthProvider;
 ...
 *
 * 연관관계:
 * - Account(N) : Company(1) - Account가 주인, @JoinColumn 사용
 * - Account(1) : Profile(1) - Account가 비주인, mappedBy 사용
 */
@Entity(name = "account")
@Table(
        indexes = {
...
public class Account extends BaseEntity {
    ...
    private Type accountType;  // 회원 계정 유형 (PERSONAL, BUSINESS) - 컨버터 자동 적용

    @Column(length = 100)
    private String position;  // 기업 계정인 경우 담당 직책 (예: HR Manager, CTO 등)

    @Column(nullable = true)
    private OAuthProvider oauthProvider;  // OAuth 제공자 (GOOGLE, KAKAO, NAVER 등) - 컨버터 자동 적용
    ...
    @Column(nullable = false)
    private Status status;  // 계정 상태 (ACTIVE, INACTIVE, PENDING_APPROVAL, REJECTED) - 컨버터 자동 적용
    ...
    public boolean isPersonalAccount() {
        return this.accountType == Type.PERSONAL;
    }
